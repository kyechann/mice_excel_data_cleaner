...
MICE 행사 종료 후 수집된 설문 데이터를 기반으로, **평점(0–10점) 및 리뷰 텍스트를 한눈에 분석**할 수 있는 전용 대시보드가 추가되었습니다.

#### 1) 만족도 분석

- 응답자를 **긍정(9–10점) / 중립(7–8점) / 부정(0–6점)**으로 자동 분류  
- 각 그룹별 **인원 수 및 비율**, 그리고 **NPS(Net Promoter Score)** 계산  
- 평점(0–10점) 분포를 막대그래프로 시각화하여 **만족도 분포를 직관적으로 파악**

![Satisfaction Dashboard](assets/review_rating_dashboard.png)

#### 2) 리뷰(코멘트) 키워드 분석

- 모든 리뷰 텍스트를 합쳐서 **워드 클라우드**로 시각화 → 자주 언급된 키워드를 직관적으로 확인  
- 평점 구간별(긍정/중립/부정)로 핵심 키워드를 분리해,  
  **"만족한 이유" / "보통인 이유" / "불만족 포인트"**를 한번에 비교  
- 군집별 키워드를 카드 형태로 정리해, 기획/운영 개선 인사이트 도출에 도움
