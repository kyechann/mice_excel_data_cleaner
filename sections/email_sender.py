...
감사합니다."""
        ...
        with t_send:
            ...
            selected_df = ed3.loc[list(selected_ids)].reset_index(drop=True) if selected_count else pd.DataFrame(columns=send3.columns)
            ...
            st.markdown("---")
            st.markdown(f"#### 👀 선택 대상 미리보기 (상위 10명) — 현재 {selected_count:,}명 선택됨")
            if selected_count == 0:
                st.info("선택 대상이 없습니다. ④ 탭에서 체크하면 여기에 바로 미리보기가 보입니다.")
            else:
                ...
                for cand in ["이름", "성명", "Name", "name"]:
                    if cand in selected_df.columns:
                        ...
                        break
                if email_col in selected_df.columns:
                    cols_for_preview.append(email_col)
                for extra in ["소속", "회사", "참가구분"]:
                    if extra in selected_df.columns:
                        cols_for_preview.append(extra)
                        if len(cols_for_preview) >= 4:
                            break
                if not cols_for_preview:
                    cols_for_preview = list(selected_df.columns)[:4]
                st.dataframe(selected_df[cols_for_preview].head(10), use_container_width=True, hide_index=True)

            if "생성된_메시지" not in send3.columns:
                st.info("먼저 ② 템플릿 탭에서 템플릿 적용으로 '생성된_메시지'를 만든 뒤 발송하세요.")
                return
            ...
            if st.button("테스트 발송 실행 (최대 5명)", use_container_width=True, key=f"{kp}_TEST_SEND"):
        ...
        with t_pick:
            ...
            with top4:
                ...
                if st.button("📌 유효 이메일만 선택", use_container_width=True, key=f"{kp}_SEL_VALID_EMAIL"):
                    email_col = st.session_state.get(f"{kp}_EMAIL_COL")
                    if not email_col or email_col not in send4.columns:
                        mail_cols = [c for c in send4.columns if ("이메일" in str(c)) or ("email" in str(c).lower())]
                        if mail_cols:
                            email_col = mail_cols[0]
                    if not email_col or email_col not in send4.columns:
                        st.warning("이메일 컬럼을 찾지 못했습니다. ③ 탭에서 이메일 컬럼을 먼저 선택하세요.")
                    else:
                        mask = ed4[email_col].apply

                        st.session_state[SELECTED_IDS_KEY] = set(ed4.index[mask].astype(str))
                        st.session_state[VER_KEY] += 1
                        st.rerun()
            ...
            st.session_state[SELECTED_IDS_KEY] = set(edited_df.index[edited_df["선택"] == True].astype(str))
            ...
            if selected_rows.empty:
                ...
                xlsx_bytes = _df_to_xlsx_bytes(selected_rows.drop(columns=["__rowid__"], errors="ignore"), sheet_name="selected")
                st.download_button(
                    "⬇️ 선택 엑셀 다운로드",
                    data=xlsx_bytes,
                    file_name="selected.xlsx",
